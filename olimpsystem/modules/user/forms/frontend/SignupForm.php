...
class SignupForm extends Model
{
    ...
    public function rules()
    {
        return [
            ...
            ['firstname','trim'],
            ['firstname','required'],
            ['firstname', 'match', 'pattern' => '#^[\w_-]+$#is'],
            ['lastname','trim'],
            ['lastname','required'],
            ['lastname', 'match', 'pattern' => '#^[\w_-]+$#is'],
            ['education','trim'],
            ['education','required'],
    ...
    public function attributeLabels()
    {
        return [
            ...
            'firstname' => Module::t('module', 'REGISTRATION_FIRSTNAME'),
            'lastname' => Module::t('module', 'REGISTRATION_LASTNAME'),
            'education' => Module::t('module','REGISTRATION_EDUCATION'),
        ...
        if ($this->validate()) {
            ...
            if ($user->save()) {
                Yii::$app->mailer->compose(['text' => '@app/modules/user/mails/emailConfirm'], ['user' => $user])
                    ...
                    ->setTo($this->email)
                    ->setSubject('Подтверждение почтового ящика для ' . Yii::$app->name)
                    ->send();
